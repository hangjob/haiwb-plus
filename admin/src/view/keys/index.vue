                            ...
                            <n-tab-pane name="查询数据">
                                <n-form style="margin-bottom: -24px" label-placement="left" label-align="right"
                                    ...
                                    <n-grid cols="24" x-gap="10" item-responsive responsive="screen">
                                        ...
                                        <n-grid-item span="24 m:12 l:4">
                                            <n-form-item label="关键词名称" path="title">
...
export default defineComponent({
    setup() {
        ...
        const compHandle = reactive({
            ...
            del(row) {
                apis['/admin/keys/destroy']({where: {id: row.id}}).then(() => {
            ...
            dels() {
                if (compData.checkedRowKeys.length) {
                    dialog.warning({
                        ...
                        negativeText: '不确定',
                        onPositiveClick: () => {
                            apis['/admin/keys/destroy']({where: {id: [compData.checkedRowKeys]}}).then(() => {
                                compHandle.getTableData()
                            })
                        },
                        onNegativeClick: () => {
            ...
            edit(row) {
                router.push("/keys/edit/" + row.id)
            },
            add() {
                router.push("/keys/add")
            },
            check(rowKeys) {
