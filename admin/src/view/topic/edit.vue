        ...
        <n-grid-item span="24 m:24 l:24">
            <n-space :wrap-item="false">
                <n-card :segmented="{content: true,footer: 'soft'}" header-style="font-size:14px;" title="添加菜单">
                    ...
                    >
                        <n-form-item label="二级分类" path="pid">
                            <n-select
                        ...
                        <n-form-item label="分类名称" path="title">
                            <n-input v-model:value="compData.from.title" placeholder="请输入分类名称"/>
                        </n-form-item>
    ...
    setup() {
        ...
        compHandle.validate = (e)=>{
            ...
            formRef.value?.validate((errors) => {
                if (!errors) {
                    apis['/admin/topic/update']({id: route.params.id,...compData.from}).then(()=>{
                        compHandle.back()
                    })
                }
            })
        ...
        apis['/admin/topic/find']({id:route.params.id}).then((res) => {
